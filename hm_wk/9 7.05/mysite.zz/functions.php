...
function getSearchArticle($query)
{
    ...
    if ($db) {
        $sql = "SELECT *
                  FROM `articles` WHERE `title` LIKE '%$query%'
                  OR `sub_title` LIKE '%$query%' OR `content` LIKE '%$query%'";
        return $db->query($sql)->fetchAll(PDO::FETCH_ASSOC);
    }
    return false;
}
...
function viewSearchForm()
{
   ...
   // } elseif (strpos($_SERVER['REQUEST_URI'], 'singlearticle')) {
   //     echo '<form action="" method="" class="form-inline my-2 my-lg-0">
